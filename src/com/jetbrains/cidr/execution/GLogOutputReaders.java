...
public abstract class GLogOutputReaders
{
    ...
    private volatile List<MyLogReader> myReaders;
    private final File myLogDir;
    private final String myLogName;
    
    public GLogOutputReaders(final File myLogDir, final String myLogName) {
        this.myReaders = new LinkedList<MyLogReader>();
        this.myLogDir = myLogDir;
    ...
    public void close() {
        final List<MyLogReader> myReaders = this.myReaders;
        this.myReaders = Collections.emptyList();
        final Iterator<MyLogReader> iterator = myReaders.iterator();
        while (iterator.hasNext()) {
            iterator.next().c();
        }
        for (final MyLogReader myLogReader : myReaders) {
            try {
                myLogReader.d();
            }
        ...
        final Iterator<MyLogReader> iterator3 = myReaders.iterator();
        while (iterator3.hasNext()) {
            final File access$300 = iterator3.next().a();
            try {
                if (!access$300.exists()) {
                    continue;
                }
                FileUtil.delete(access$300);
            }
    ...
    protected abstract void onTextAvailable(@NotNull final String p0, @NotNull final LogType p1);
    
    static {
        LOG = Logger.getInstance((Class)GLogOutputReaders.class);
    }
