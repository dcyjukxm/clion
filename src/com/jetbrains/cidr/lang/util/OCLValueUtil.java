...
{
    public static boolean isLValue(@NotNull final OCExpression ocExpression) {
        try {
            if (ocExpression == null) {
                throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "expression", "com/jetbrains/cidr/lang/util/OCLValueUtil", "isLValue"));
            }
        }
        catch (IllegalArgumentException ex) {
            throw a(ex);
        }
        final LValueChecker lValueChecker = new LValueChecker();
        lValueChecker.process(ocExpression);
        return lValueChecker.isLValue();
    }
    
    public static boolean isAssignmentLHS(@NotNull final OCExpression ocExpression) {
        try {
            if (ocExpression == null) {
                throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "expression", "com/jetbrains/cidr/lang/util/OCLValueUtil", "isAssignmentLHS"));
            }
        }
        catch (IllegalArgumentException ex) {
            throw a(ex);
        ...
        final LValueChecker lValueChecker = new LValueChecker();
        lValueChecker.process(ocExpression);
        return lValueChecker.isAssignmentLHS();
    }
