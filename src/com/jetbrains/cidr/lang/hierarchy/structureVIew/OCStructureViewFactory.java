...
public class OCStructureViewFactory implements PsiStructureViewFactory
{
    public StructureViewBuilder getStructureViewBuilder(@NotNull final PsiFile psiFile) {
        try {
            if (psiFile == null) {
                throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "psiFile", "com/jetbrains/cidr/lang/hierarchy/structureVIew/OCStructureViewFactory", "getStructureViewBuilder"));
            }
        }
        catch (IllegalArgumentException ex) {
            throw a(ex);
        }
        return (StructureViewBuilder)new TreeBasedStructureViewBuilder() {
            @NotNull
            public StructureViewModel createStructureViewModel(@Nullable final Editor editor) {
                OCStructureViewModel ocStructureViewModel;
                try {
                    ocStructureViewModel = new OCStructureViewModel(psiFile, editor);
