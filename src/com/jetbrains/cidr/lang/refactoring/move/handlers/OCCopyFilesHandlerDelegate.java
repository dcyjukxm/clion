...
public abstract class OCCopyFilesHandlerDelegate<M, R, G extends R, P> extends CopyFilesOrDirectoriesHandler
{
    protected abstract void addTargets(final Set<String> p0, final R p1);
    
    protected abstract void addFiles(final PsiDirectory p0, final G p1, final List<VirtualFile> p2, final Set<String> p3);
    
    protected abstract boolean skipElement(final PsiElement p0);
    
    protected abstract OCCopyMoveProcessor.Helper<M, R, G, P> getHelper();
    
    @Override
    public void doCopy(final PsiElement[] array, PsiDirectory targetDirectory) {
        if (array.length == 0) {
            return;
        }
        String newName = null;
        final PsiDirectory psiDirectory = (array[0] instanceof PsiDirectory) ? ((PsiDirectory)array[0]).getParentDirectory() : array[0].getContainingFile().getContainingDirectory();
        if (targetDirectory == null || targetDirectory.equals(psiDirectory)) {
            final CopyFilesOrDirectoriesDialog copyFilesOrDirectoriesDialog = new CopyFilesOrDirectoriesDialog(array, psiDirectory, array[0].getProject(), false);
        ...
        new MyProcessor(array, newName).process(array[0].getProject(), (PsiElement)targetDirectory);
    }
    
    public void doCloneForTest(final PsiElement psiElement) {
        new MyProcessor(new PsiElement[] { psiElement }, "CloneTest") {
            final /* synthetic */ Map val$filesMap = new HashMap();
        ...
        }.process(psiElement.getProject(), (PsiElement)((psiElement instanceof PsiDirectory) ? ((PsiDirectory)psiElement).getParentDirectory() : ((PsiFile)psiElement).getContainingDirectory()));
        final Map<K, OCFile> map;
        final Iterator<OCFile> iterator;
        OCFile ocFile;
        ApplicationManager.getApplication().runWriteAction(() -> {
            map.keySet().iterator();
            while (iterator.hasNext()) {
                ocFile = iterator.next();
                OCChangeUtil.changeText(psiElement.getProject(), (PsiFile)ocFile, 0, ocFile.getTextLength(), map.get(ocFile).getText(), true);
            }
        });
    }
    
    @Override
    public void doClone(final PsiElement psiElement) {
        final PsiDirectory psiDirectory = (psiElement instanceof PsiDirectory) ? ((PsiDirectory)psiElement).getParentDirectory() : ((PsiFile)psiElement).getContainingDirectory();
        final PsiElement[] array = { psiElement };
        final CopyFilesOrDirectoriesDialog copyFilesOrDirectoriesDialog = new CopyFilesOrDirectoriesDialog(array, null, psiElement.getProject(), true);
        if (copyFilesOrDirectoriesDialog.showAndGet()) {
            new MyProcessor(array, copyFilesOrDirectoriesDialog.getNewName()).process(psiElement.getProject(), (PsiElement)psiDirectory);
        }
    }
