...
public abstract class OCMoveFilesHandlerDelegate<M, R, G extends R, P, RTP, RPP> extends MoveFilesOrDirectoriesHandler
{
    @Override
    public void doMove(final Project project, final PsiElement[] array, final PsiElement psiElement, final MoveCallback moveCallback) {
        if (psiElement == null) {
            for (int length = array.length, i = 0; i < length; ++i) {
                if (this.skipElement(array[i])) {
                    return;
                }
            }
            final MoveFilesOrDirectoriesDialog moveFilesOrDirectoriesDialog = new MoveFilesOrDirectoriesDialog(project, new MoveFilesOrDirectoriesDialog.Callback() {
                @Override
                public void run(final MoveFilesOrDirectoriesDialog moveFilesOrDirectoriesDialog) {
                    OCMoveFilesHandlerDelegate.this.doMove(project, array, (PsiElement)moveFilesOrDirectoriesDialog.getTargetDirectory(), moveCallback);
                    moveFilesOrDirectoriesDialog.close(1);
                }
            });
            moveFilesOrDirectoriesDialog.setData(array, MoveFilesOrDirectoriesUtil.getInitialTargetDirectory(null, array), "refactoring.moveFile");
            moveFilesOrDirectoriesDialog.show();
            return;
        }
        new OCCopyMoveProcessor<M, R, G, P>(array) {
            @Override
        ...
        }.process(project, psiElement);
    }
    
    protected abstract boolean isFilesView(final Project p0);
    
    protected abstract boolean skipElement(final PsiElement p0);
    
    protected abstract RPP getReferencePathProviderForCopying(final String p0, final String p1);
    
    protected abstract RPP getIdPathProvider();
    
    protected abstract boolean isValid(final R p0);
    
    protected abstract String getPath(final R p0);
    
    protected abstract PsiFileSystemItem findPsiElement(final R p0);
    
    protected abstract void addFiles(final Project p0, final P p1, final List<VirtualFile> p2, final G p3, final PsiDirectory p4);
    
    protected abstract void removePaths(final THashSet<String> p0, final P p1);
    
    protected abstract OCCopyMoveProcessor.Helper<M, R, G, P> getHelper();
    
    @NotNull
    protected abstract List<?> moveReference(@NotNull final M p0, @NotNull final R p1, @NotNull final RPP p2, @NotNull final G p3, @NotNull final RTP p4);
    
    @Nullable
    protected abstract RTP getTargetProvider(@NotNull final List<R> p0, @NotNull final G p1);
