...
public class OCMemberInheritorsSearch<T extends OCMemberSymbol> extends QueryFactory<T, SearchParameters>
{
    public static final OCMemberInheritorsSearch INSTANCE;
    
    public static <T extends OCMemberSymbol> SearchParameters<T> getParameters(final T t) {
        if (t == null) {
            return new SearchParameters<T>();
        }
        final Project project = t.getProject();
        return new SearchParameters<T>(project, GlobalSearchScope.allScope(project), t.getName(), (OCClassSymbol)((OCSymbolWithParent<T, OCClassSymbol>)t).getParent(), (Class)t.getClass(), (t instanceof OCMethodSymbol || t instanceof OCPropertySymbol) ? (t.isStatic() ? OCObjectTypeContext.StaticMode.STATIC : OCObjectTypeContext.StaticMode.INSTANCE) : OCObjectTypeContext.StaticMode.NO_MATTER);
    }
    
    public static SearchParameters<OCMethodSymbol> getParameters(final OCMethod ocMethod) {
        final Project project = ocMethod.getProject();
        return new SearchParameters<OCMethodSymbol>(project, GlobalSearchScope.allScope(project), ocMethod.getSelector(), (OCClassSymbol)ApplicationManager.getApplication().runReadAction((Computable)new Computable<OCClassSymbol>() {
            public OCClassSymbol compute() {
                return ocMethod.getContainingClass().getSymbol();
            }
        }), (Class)OCMethodSymbol.class, ocMethod.isInstanceMethod() ? OCObjectTypeContext.StaticMode.INSTANCE : OCObjectTypeContext.StaticMode.STATIC);
    }
    
    public static <T extends OCMemberSymbol> SearchParameters<T> getParameters(final String s, final OCClassSymbol ocClassSymbol, final Project project, final Class<? extends T> clazz, final OCObjectTypeContext.StaticMode staticMode) {
        return new SearchParameters<T>(project, GlobalSearchScope.allScope(project), s, ocClassSymbol, (Class)clazz, staticMode);
    }
    
    public static <T extends OCMemberSymbol> Query<T> search(final SearchParameters<T> searchParameters) {
        if (searchParameters.getSelectorName() == null) {
            return (Query<T>)new EmptyQuery();
        }
        return (Query<T>)OCMemberInheritorsSearch.INSTANCE.createQuery((Object)searchParameters);
    }
    
    static {
        (INSTANCE = new OCMemberInheritorsSearch()).registerExecutor((QueryExecutor)new DefaultExecutor());
    }
