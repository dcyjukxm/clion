...
public class OCFunctionInheritorsSearch extends QueryFactory<OCFunctionSymbol, SearchParameters>
{
    public static final OCFunctionInheritorsSearch INSTANCE;
    
    public static SearchParameters getParameters(final OCSymbolWithQualifiedName ocSymbolWithQualifiedName, final OCFile ocFile, final boolean b) {
        ...
        final OCQualifiedName ocQualifiedName = (resolvedQualifiedName != null) ? resolvedQualifiedName.getQualifier() : OCQualifiedName.GLOBAL;
        final GlobalSearchScope allScope = GlobalSearchScope.allScope(ocSymbolWithQualifiedName.getProject());
        SearchParameters searchParameters;
        if (b) {
            searchParameters = SearchParameters.getSearchFunctionBySignatureParameters(ocFile, allScope, ocSymbolWithQualifiedName.getName(), ocSymbolWithQualifiedName.getType(), ocQualifiedName);
        }
        else {
            searchParameters = SearchParameters.getSearchFunctionByNameParameters(ocFile, allScope, ocSymbolWithQualifiedName.getName(), ocQualifiedName);
        }
        if (ocSymbolWithQualifiedName instanceof OCFunctionSymbol && ((OCFunctionSymbol)ocSymbolWithQualifiedName).isFriend() && !((OCFunctionSymbol)ocSymbolWithQualifiedName).isCppOperator()) {
            searchParameters.myIsFriend = true;
        }
        searchParameters.myIsFriendOrStatic = ocSymbolWithQualifiedName.isFriendOrStatic();
        return searchParameters;
    }
    
    public static SearchParameters getParameters(final OCFunctionDeclaration ocFunctionDeclaration, final boolean b) {
        return getParameters(ocFunctionDeclaration.getSymbol(), ocFunctionDeclaration.getContainingOCFile(), b);
    }
    
    public static Query<OCFunctionSymbol> search(final SearchParameters searchParameters) {
        return search(searchParameters, null);
    }
    
    public static Query<OCFunctionSymbol> search(final SearchParameters searchParameters, @Nullable final MultiMap<OCQualifiedName, OCQualifiedName> multiMap) {
        if (searchParameters.getFunctionName() == null) {
            return (Query<OCFunctionSymbol>)new EmptyQuery();
        }
        searchParameters.myInheritenceCache = multiMap;
        return (Query<OCFunctionSymbol>)OCFunctionInheritorsSearch.INSTANCE.createQuery((Object)searchParameters);
    }
    
    public static Query<OCFunctionSymbol> search(final OCFunctionSymbol ocFunctionSymbol, final OCFile ocFile, final boolean b) {
        return search(getParameters(ocFunctionSymbol, ocFile, b));
    }
    
    static {
        (INSTANCE = new OCFunctionInheritorsSearch()).registerExecutor((QueryExecutor)new DefaultExecutor());
    }
