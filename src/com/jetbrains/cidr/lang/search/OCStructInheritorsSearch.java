...
public class OCStructInheritorsSearch extends QueryFactory<OCStructSymbol, SearchParameters>
{
    public static final OCStructInheritorsSearch INSTANCE;
    
    public static Query<OCStructSymbol> search(final OCStruct ocStruct) {
        final Project project = ocStruct.getProject();
        final OCStructSymbol ocStructSymbol = ocStruct.getSymbol();
        try {
            if (ocStructSymbol != null) {
                final OCQualifiedName resolvedQualifiedName = ocStructSymbol.getResolvedQualifiedName();
                return search(resolvedQualifiedName, GlobalSearchScope.allScope(project), ocStruct.getContainingFile(), project);
            }
        }
        catch (IllegalArgumentException ex) {
            throw a(ex);
        }
        final OCQualifiedName resolvedQualifiedName = null;
        return search(resolvedQualifiedName, GlobalSearchScope.allScope(project), ocStruct.getContainingFile(), project);
    }
    
    public static Query<OCStructSymbol> search(final OCStructSymbol ocStructSymbol, @NotNull final PsiElement psiElement) {
        try {
            if (psiElement == null) {
        ...
        final Project project = ocStructSymbol.getProject();
        return search(ocStructSymbol.getResolvedQualifiedName(), GlobalSearchScope.allScope(project), psiElement.getContainingFile(), project);
    }
    
    public static Query<OCStructSymbol> search(final OCQualifiedName ocQualifiedName, final GlobalSearchScope globalSearchScope, final PsiFile psiFile, final Project project) {
        return (Query<OCStructSymbol>)OCStructInheritorsSearch.INSTANCE.createQuery((Object)new SearchParameters(globalSearchScope, ocQualifiedName, psiFile, project));
    }
    
    static {
        (INSTANCE = new OCStructInheritorsSearch()).registerExecutor((QueryExecutor)new DefaultExecutor());
    }
