...
public class OCMethodImplementationCompletionContributor extends OCCompletionContributorBase
{
    public static final ElementPattern<PsiElement> SELECTOR_PART_DECLARATION;
    ...
    public OCMethodImplementationCompletionContributor() {
            ...
            protected void addCompletions(final String s, @NotNull final OCCompletionParameters ocCompletionParameters, final ProcessingContext processingContext, final CompletionResultSet set) {
                ...
                if (parent instanceof OCMethodSelectorPart) {
                    ...
                    if (parent2 instanceof OCImplementation) {
                        ...
                        a(set, ocCompletionParameters, false, new MethodSelectorCompletionContributor.Context(string, null, ocObjectTypeContext, (Condition<OCMethodSymbol>)and));
                    }
                }
            }
        ...
        this.register(CompletionType.BASIC, OCMethodImplementationCompletionContributor.SELECTOR_PART_DECLARATION, ocCompletionProvider);
        this.register(CompletionType.SMART, OCMethodImplementationCompletionContributor.SELECTOR_PART_DECLARATION, ocCompletionProvider);
    }
    
    private static int a(final CompletionResultSet set, final OCCompletionParameters ocCompletionParameters, final boolean b, final MethodSelectorCompletionContributor.Context context) {
        final int[] array = { 0 };
        final Processor<OCMethodSymbol> processor = MethodSelectorCompletionContributor.getProcessor(ocCompletionParameters, set, array, context);
        context.getReceiverContext().getType().processMembers(OCMethodSymbol.class, processor);
        if (!b) {
            final OCType resolve = OCReferenceType.fromText("NSObject").resolve(ocCompletionParameters.getOriginalFile());
            if (resolve instanceof OCObjectType) {
                ((OCObjectType)resolve).processMembers(OCMethodSymbol.class, processor);
            }
        }
        return array[0];
    }
    ...
    static {
        SELECTOR_PART_DECLARATION = (ElementPattern)PlatformPatterns.psiElement((IElementType)OCTokenTypes.IDENTIFIER).withParent((Class)OCMethodSelectorPart.class);
    }
}
