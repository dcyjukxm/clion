...
public class QualifiedSelectorCompletionContributor extends OCCompletionContributorBase
{
    public QualifiedSelectorCompletionContributor() {
        this.register(CompletionType.BASIC, (ElementPattern<? extends PsiElement>)PlatformPatterns.psiElement().withParent((Class)OCQualifiedExpression.class), new OCCompletionProvider() {
            @Override
    ...
    public String handleEmptyLookup(@NotNull final CompletionParameters completionParameters, final Editor editor) {
        try {
            if (completionParameters == null) {
                throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "parameters", "com/jetbrains/cidr/lang/editor/completion/QualifiedSelectorCompletionContributor", "handleEmptyLookup"));
            }
        }
        catch (IllegalArgumentException ex) {
            throw a(ex);
        }
        final PsiElement parent = completionParameters.getPosition().getParent();
        if (parent instanceof OCQualifiedExpression) {
            final OCQualifiedExpression ocQualifiedExpression = (OCQualifiedExpression)parent;
            final OCType resolve = ocQualifiedExpression.getQualifier().getType().resolve((PsiFile)ocQualifiedExpression.getContainingOCFile());
            try {
                if (resolve.getTerminalType() instanceof OCObjectType) {
                    return "No suggestions for properties of class " + resolve.getTerminalType().getName((PsiElement)ocQualifiedExpression);
                }
            }
            catch (IllegalArgumentException ex2) {
                throw a(ex2);
            }
            return "No suggestions for members of " + resolve.getName((PsiElement)ocQualifiedExpression);
        }
