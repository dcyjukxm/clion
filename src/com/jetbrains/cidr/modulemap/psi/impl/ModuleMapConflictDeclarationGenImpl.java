...
public class ModuleMapConflictDeclarationGenImpl extends ModuleMapPsiElementImpl implements ModuleMapConflictDeclaration
{
    public ModuleMapConflictDeclarationGenImpl(final ASTNode astNode) {
        super(astNode);
    }
    ...
    public void accept(@NotNull final ModuleMapVisitor moduleMapVisitor) {
        try {
            if (moduleMapVisitor == null) {
                throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "visitor", "com/jetbrains/cidr/modulemap/psi/impl/ModuleMapConflictDeclarationGenImpl", "accept"));
            }
        }
        catch (IllegalArgumentException ex) {
            throw a(ex);
        }
        moduleMapVisitor.visitConflictDeclaration(this);
    }
