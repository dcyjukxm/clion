...
{
    public FilesModificationsListener install(@NotNull final Project project, @NotNull final Disposable disposable) {
        try {
            if (project == null) {
                throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "project", "com/jetbrains/cidr/FilesModificationsListener", "install"));
            }
        }
    ...
    protected abstract void watchedFilesChanged(@NotNull final Set<File> p0, @NotNull final Set<VirtualFile> p1, final boolean p2, final boolean p3);
